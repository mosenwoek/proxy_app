 ...
 * ✨ 【功能概览】
 * • 三尺寸自适应独立排版
 * • 核心引擎：内网/本地/节点精准 IP 穿透识别
 * • 智能解析：突破代理工具嵌套层级
 * • 稳健防护：针对 Fallback 策略组深度调优
 * • 策略指定：支持通过环境变量 POLICY 指定策略组/节点
 *
...
export default async function (ctx) {
  ...
  // ==========================================
  // 在 Egern 小组件环境变量中配置：
  // POLICY=策略组名 或 POLICY=节点名
  // 示例：POLICY=Tiktok 或 POLICY=日本节点
  const env = ctx.env || {};
  const POLICY = clean(env.POLICY);
  const POLICY_LABEL = POLICY || "默认策略";
  ...
  if (isSmall || isMedium) {
    ...
    const httpGet = async (url) => {
      try {
        ...
        const resp = await ctx.http.get(url, requestOptions({ headers: commonHeaders, timeout: 3500 }));
    ...
    async function checkYouTube() { const res = await ctx.http.get(`https://www.youtube.com/generate_204`, requestOptions({ timeout: 3500, headers: commonHeaders })).catch(() => null); return { code: res?.status === 204 ? 'OK' : 'ERR' }; }
    async function checkNetflix() { const res = await ctx.http.get(`https://www.netflix.com/generate_204`, requestOptions({ timeout: 3500, headers: commonHeaders, followRedirect: false })).catch(() => null); return { code: (res?.status === 204 || res?.status === 200) ? 'OK' : 'ERR' }; }
    async function checkDisney() { const res = await ctx.http.get(`https://www.disneyplus.com/`, requestOptions({ timeout: 4500, headers: commonHeaders, followRedirect: false })).catch(() => null); return { code: (res && res.status !== 403) ? 'OK' : 'ERR' }; }
    async function checkSpotify() { const res = await ctx.http.get(`https://open.spotify.com/`, requestOptions({ timeout: 3500, headers: commonHeaders, followRedirect: false })).catch(() => null); return { code: res && res.status === 200 ? 'OK' : 'ERR' }; }
    async function checkChatGPT() { const res = await ctx.http.get(`https://chatgpt.com/`, requestOptions({ timeout: 4500, headers: commonHeaders, followRedirect: false })).catch(() => null); return { code: (res && (res.status === 200 || res.status === 302 || res.status === 401 || res.status === 404)) ? 'OK' : 'ERR' }; }
    async function checkClaude() { const res = await ctx.http.get(`https://api.anthropic.com/`, requestOptions({ timeout: 4500, headers: commonHeaders, followRedirect: false })).catch(() => null); return { code: (res && (res.status === 404 || res.status === 401 || res.status === 200)) ? 'OK' : 'ERR' }; }
    async function checkGemini() {
      const startTime = Date.now();
      try {
        const res = await ctx.http.get("https://generativelanguage.googleapis.com/v1beta/models", requestOptions({
          headers: {
    ...
    async function checkGrok() { const res = await ctx.http.get(`https://grok.com/`, requestOptions({ timeout: 3500, headers: commonHeaders, followRedirect: false })).catch(() => null); return { code: res && res.status === 200 ? 'OK' : 'ERR' }; }
    ...
    try {
      ...
      if (isSmall) {
        ...
        // 如果指定了策略，在标题中显示
        const displayTitle = POLICY ? `${smallTitleStr} [${POLICY}]` : smallTitleStr;
        ...
        return {
          ...
          children: [
            mkRow([
              ...
              mkSpacer(6),
              mkText(displayTitle, 13, "heavy", C.main, { maxLines: 1, minScale: 0.5, flex: 1 }),
      ...
      const buildHeader = () => mkRow([
        ...
        ...(POLICY ? [mkText(`[${POLICY}]`, 10, "bold", C.purple, { maxLines: 1, minScale: 0.5 })] : []),
    ...
    const httpGet = async (url) => {
      const start = Date.now();
      try {
        const resp = await ctx.http.get(url, requestOptions({ headers: commonHeaders, timeout: TIMEOUT_MS }));
        const text = await resp.text();
        const json = JSON.parse(text);
        return { data: json.data || json, ping: Date.now() - start };
      } catch (e) { return { data: {}, ping: 0 }; }
    ...
    async function checkYouTube() { const res = await ctx.http.get(`https://www.youtube.com/generate_204`, requestOptions({ timeout: TIMEOUT_MS, headers: commonHeaders })).catch(() => null); return { code: res?.status === 204 ? 'OK' : 'ERR' }; }
    async function checkNetflix() { const res = await ctx.http.get(`https://www.netflix.com/generate_204`, requestOptions({ timeout: TIMEOUT_MS, headers: commonHeaders, followRedirect: false })).catch(() => null); return { code: (res?.status === 204 || res?.status === 200) ? 'OK' : 'ERR' }; }
    async function checkDisney() { const res = await ctx.http.get(`https://www.disneyplus.com/`, requestOptions({ timeout: 4500, headers: commonHeaders, followRedirect: false })).catch(() => null); return { code: (res && res.status !== 403) ? 'OK' : 'ERR' }; }
    async function checkSpotify() { const res = await ctx.http.get(`https://open.spotify.com/`, requestOptions({ timeout: TIMEOUT_MS, headers: commonHeaders, followRedirect: false })).catch(() => null); return { code: res && res.status === 200 ? 'OK' : 'ERR' }; }
    async function checkChatGPT() { const res = await ctx.http.get(`https://chatgpt.com/`, requestOptions({ timeout: 4500, headers: commonHeaders, followRedirect: false })).catch(() => null); return { code: (res && (res.status === 200 || res.status === 302 || res.status === 401 || res.status === 404)) ? 'OK' : 'ERR' }; }
    async function checkClaude() { const res = await ctx.http.get(`https://api.anthropic.com/`, requestOptions({ timeout: 4500, headers: commonHeaders, followRedirect: false })).catch(() => null); return { code: (res && (res.status === 404 || res.status === 401 || res.status === 200)) ? 'OK' : 'ERR' }; }
    async function checkGemini() {
      const startTime = Date.now();
    ...
    async function checkGrok() { const res = await ctx.http.get(`https://grok.com/`, requestOptions({ timeout: TIMEOUT_MS, headers: commonHeaders, followRedirect: false })).catch(() => null); return { code: res && res.status === 200 ? 'OK' : 'ERR' }; }
    ...
    const headerRow = mkRow([
      ...
      mkSpacer(),
      mkRow([ mkIcon('shield.fill', C.purple, 9), mkSpacer(4), mkText(POLICY_LABEL, 9, C.textMain, 'bold', { maxLines: 1 }) ], { padding: [3, 8], backgroundColor: C.badgeBg, borderRadius: 6 }),
      mkSpacer(8),
